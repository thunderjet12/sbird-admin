...
if ( ! class_exists( 'Dependency_Loader' ) ) {
	 ...
	 * @since      1.0.0
	 * @package    Sbird_Admin
	 * @subpackage Sbird_Admin/includes
	...
	class Dependency_Loader {
		...
		public function load_dependencies( $class ) {
			...
			if ( 'Sbird_Admin' !== $parts[0] ) {
				return;
			}

			// Remove 'Sbird_Admin' from parts.
			array_shift( $parts );
			...
			$file_path = Sbird_Admin::get_plugin_path() . implode( '/', $parts ) . $class_file_name;
			...
			$file_path = Sbird_Admin::get_plugin_path() . implode( '/', $parts ) . $trait_file_name;
		...
		protected function load_registries() {
			require_once( Sbird_Admin::get_plugin_path() . 'core/registry/trait-base-registry.php' );
			require_once( Sbird_Admin::get_plugin_path() . 'core/registry/class-controller.php' );
			require_once( Sbird_Admin::get_plugin_path() . 'core/registry/class-model.php' );
		}
		...
		protected function load_core() {
			...
			foreach ( glob( Sbird_Admin::get_plugin_path() . 'core/*.php' ) as $path ) {
				require_once $path;
			}
		}
